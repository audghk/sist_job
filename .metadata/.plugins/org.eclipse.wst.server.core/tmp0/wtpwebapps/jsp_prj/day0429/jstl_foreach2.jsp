...
<c:forEach var="lunch" items="${ lunch }" varStatus="i">
	...
	<c:if test="${ i.last }">
	<br/>
		<c:out value="${ i.count }"/>건의 메뉴가 제안되었습니다.<br/>
	</c:if>
</c:forEach>
...
<input type="text" name="name" value="${ name }" style="width:150px" <c:if test="${ i.count == 3}">readonly</c:if>/><br/>
</c:forEach>
...
deptList.add(new TestVO(50,"Solution","나주"));
