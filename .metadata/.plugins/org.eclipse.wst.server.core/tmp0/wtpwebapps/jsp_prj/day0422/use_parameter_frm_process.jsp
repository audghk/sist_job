...
request.setCharacterEncoding("UTF-8");//한글 안깨짐. 이거 하나만 써도됨. GET방식에는 적용되지 않는다.
...
String chk=request.getParameter("chk");//<input type="checkbox" name="chk"> checked 된 상태에서만 값이 넘어온다.
...
String str=new String(txt.getBytes("8859_1"),"UTF-8");//한글이 안깨짐
//URLDecoder class로 decoding
String str2=URLDecoder.decode(URLEncoder.encode(hid,"8859_1"),"UTF-8");//한글이 안깨짐

//name속성에 값이 같으면 배열로 처리되고, request.getParameterValues("이름")받는다.
String[] names=request.getParameterValues("name");//<input type="text" value="name">
String[] hobby=request.getParameterValues("hobby");//<input type="text" value="name">
%>
<ul>
...
//try{
if(hobby != null){
for(String hob : hobby){//checkbox는 체크하지 않으면 Parameter로 생성되지 않아 배열에 null이 들어간다.
  	//들어간다. => 반복하면 배열명.length 사용하는 코드에서 NullPointerException이 발생하게된다.%>
<%= hob %>  
<%} 
...
//}
  ...
  out.println("취미를 하나 정도 가지는게 좋을 것 같음.");
}
%>
<br/>
