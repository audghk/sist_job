...
public class RestaurantDAO {
  ...
  public List<RestaurantVO> selectAllRestaurant() throws SQLException {
    ...
    try {
      ...
      // 2. DataSource 얻기
      // 3. Connection 얻기
      conn = dbConn.getConn("jdbc/dbcp");

      // 4. 쿼리문 생성 객체 얻기(Dynamic Query)
      StringBuilder selectRestaurant = new StringBuilder();
      selectRestaurant.append(" select name, menu, lat, lng, input_date ")
          .append(" from restaurant order by input_date desc ");

      pstmt = conn.prepareStatement(selectRestaurant.toString());
      // 5. 바인드 변수에 값 설정
      // 6. 쿼리문 수행 후 결과 얻기
      ...
      while (rs.next()) {
        rVO = new RestaurantVO(rs.getString("name"), rs.getString("menu"), rs.getDouble("lat"), rs.getDouble("lng"),
